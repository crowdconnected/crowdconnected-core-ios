...
/// The entry point class of the <code>CrowdConnected</code> library.
SWIFT_CLASS("_TtC18CrowdConnectedCore14CrowdConnected")
@interface CrowdConnected : NSObject
/// A <code>CrowdConnectedDelegate</code> delegate used to return location data produced by the <code>CrowdConnected</code> library.
...
/// Used for library configurations and communication.
...
///
/// \param completion A completion representing the success or failure of the library start.
/// The format of the completion is (deviceID: String?, error: String?).
/// If the library starts correctly with the provided credentials, a non-nil device ID will be returned.
/// Id the library fails to start, a non-nil error will be returned.
///
...
/// Activate automatic background library refreshes.
- (void)activateLibraryBackgroundRefresh SWIFT_AVAILABILITY(ios,introduced=13.0);
/// Schedule the first background library refresh.
- (void)scheduleLibraryRefresh SWIFT_AVAILABILITY(ios,introduced=13.0);
@end


@class Location;

/// A protocol used to return locations produced by the library.
SWIFT_PROTOCOL("_TtP18CrowdConnectedCore22CrowdConnectedDelegate_")
@protocol CrowdConnectedDelegate
/// Called when new locations have been computed by the library.
...
/// An <code>NSObject</code> representing the accurate location of the device calculated by the library.
...
/// An <code>NSObject</code> representing the location of the device in pixel coordinates calculated by the library.
